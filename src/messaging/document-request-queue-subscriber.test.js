import { configureAndStart, stopSubscriber } from './document-request-queue-subscriber.js'
...
jest.mock('./process-document-request.js')

describe('MessageRequestQueueSubscriber', () => {
  beforeEach(() => {
    jest.clearAllMocks()
    config.set('sqs.documentRequestQueueUrl', 'http://localhost:4576/queue/document-request-queue')
  ...
  describe('configureAndStart', () => {
    ...
    it('should pass message on via OnMessage function', async () => {
      ...
      expect(processDocumentRequest).toHaveBeenCalledTimes(1)
    })
  })
