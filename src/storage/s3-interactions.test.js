...
jest.mock('../common/helpers/s3/s3-client.js')
...
describe('s3-interactions', () => {
  ...
  const mockS3Client = {
    send: jest.fn(() => Promise.resolve(mockPutObjectResponse))
  }
  ...
  describe('deleteBlob', () => {
    it('should delete a blob from the specified S3 bucket', async () => {
      ...
      expect(mockLogger.info).toHaveBeenCalledWith(`Successfully deleted blob: ${key}`)
    })
    ...
    it('should return true and print log warning if object already deleted from S3 bucket', async () => {
      ...
      expect(mockS3Client.send).toHaveBeenCalledWith(expect.any(DeleteObjectCommand))
      expect(result).toBeTruthy()
      expect(mockLogger.warn).toHaveBeenCalledWith(`Blob not found or already deleted: ${key}`)
    })
